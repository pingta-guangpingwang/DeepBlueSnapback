...
export async function startExternalApi(rootPath: string): Promise<{
  ...
  app.post('/api/v1/projects/:name/vector/ingest', async (req: Request, res: Response) => {
    try {
      const { filePaths } = req.body || {}
      if (!filePaths || !Array.isArray(filePaths) || filePaths.length === 0) {
        return res.status(400).json({ success: false, message: 'filePaths array required' })
      }
      const { ingestFiles } = await import('./vector-engine')
      const name = String(req.params.name)
      const registry = readRegistry(rootPath)
      const proj = registry.find(e => e.name === name)
      if (!proj) { res.status(404).json({ error: 'Project not found' }); return }
  ...
  app.get('/api/v1/projects/:name/vector/files', async (req: Request, res: Response) => {
    try {
      const { getIndexedFiles } = await import('./vector-engine')
  ...
  app.delete('/api/v1/projects/:name/vector/files', async (req: Request, res: Response) => {
    try {
      const { filePaths } = req.body || {}
      if (!filePaths || !Array.isArray(filePaths) || filePaths.length === 0) {
        return res.status(400).json({ success: false, message: 'filePaths array required' })
      }
      const { removeFilesFromIndex } = await import('./vector-engine')
      const name = String(req.params.name)
  ...
  app.get('/api/v1/projects/:name/vector/export', async (req: Request, res: Response) => {
    try {
      const { exportVectorIndex } = await import('./vector-engine')
  ...
  app.post('/api/v1/projects/:name/vector/import', async (req: Request, res: Response) => {
    try {
      const data = typeof req.body === 'string' ? req.body : JSON.stringify(req.body)
      const { importVectorIndex } = await import('./vector-engine')
  ...
  app.get('/api/v1/projects/:name/vector/supported', async (_req: Request, res: Response) => {
    try {
      const { getSupportedExtensions } = await import('./vector-engine')
