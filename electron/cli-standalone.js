...
program.command('import-project <src>')
    ...
    try {
        ...
        if (existing) {
            if (!existing.workingCopies.some((wc) => path.resolve(wc.path) === normalizedPath)) {
...
program.command('git-pull <projectPath>')
    ...
    try {
        ...
        try {
            await isomorphic_git_1.default.merge({ fs, dir: projectPath, ours: branch, theirs: `origin/${branch}`, author: { name: 'DBVS', email: 'dbvs@local' } });
