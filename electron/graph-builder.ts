...
import type { ArchitectureGraph, GraphNode, GraphEdge, GraphMetrics } from './graph-types'
import type { ParsedFile, ParseResult } from './ast-analyzer'
...
function aggregateStats(node: GraphNode): void {
...
function detectCircularDeps(
  ...
  for (const edge of edges) {
    if (edge.type === 'circular') continue // already marked
  ...
  function dfs(node: string, pathNodes: Set<string>): boolean {
    color.set(node, GRAY)
    pathNodes.add(node)
...
export function buildGraph(
  ...
  const fileNodeMap = new Map<string, GraphNode>() // relPath -> leaf node
  const moduleNodeMap = new Map<string, GraphNode>() // module path -> node
  ...
  const exportMap = new Map<string, string[]>() // name -> file paths that export it
  for (const file of parseResult.files) {
  ...
  for (const file of parseResult.files) {
    ...
    for (const callName of calledNames) {
      ...
      if (isBuiltinCall(callName)) continue
...
function consolidateEdges(edges: GraphEdge[]): GraphEdge[] {
...
function countOrphans(root: GraphNode, edges: GraphEdge[]): number {
...
function getMaxDepth(node: GraphNode): number {
