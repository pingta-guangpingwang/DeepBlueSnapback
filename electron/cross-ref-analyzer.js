...
exports.analyzeCrossReferences = analyzeCrossReferences;
...
function matchImportToProject(importPath, sourceFile, projects) {
    const results = [];
...
async function analyzeCrossReferences(rootPath, focusProjectName, registry) {
        ...
        for (const file of focusProject.files.slice(0, 200)) { // Limit to 200 files for perf
            ...
            for (const imp of imports) {
                ...
                for (const m of matches) {
                    // Avoid duplicates
                    if (!crossRefs.some(r => r.sourceProject === focusProjectName &&
    ...
    for (const ref of crossRefs) {
        if (!projectDeps[ref.sourceProject])
            projectDeps[ref.sourceProject] = [];
        if (!projectDeps[ref.sourceProject].includes(ref.targetProject)) {
            projectDeps[ref.sourceProject].push(ref.targetProject);
        }
        if (!projectDependents[ref.targetProject])
            projectDependents[ref.targetProject] = [];
...
async function getFileTreeSimple(dir) {
    const files = [];
