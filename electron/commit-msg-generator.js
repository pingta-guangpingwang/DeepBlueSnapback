 ...
 */
function generateCommitMessage(changes, graph) {
    ...
    for (const c of changes) {
        ...
        if (['ts', 'tsx', 'js', 'jsx', 'mjs', 'cjs'].includes(ext))
            cat = '代码';
        else if (['css', 'scss', 'less', 'svg', 'png', 'jpg'].includes(ext))
            cat = '样式/资源';
        else if (['json', 'yaml', 'yml', 'toml', 'ini', 'env'].includes(ext))
            cat = '配置';
        else if (['md', 'txt', 'rst', 'adoc'].includes(ext))
            cat = '文档';
        else if (['test', 'spec'].some(t => c.path.includes(t)))
            cat = '测试';
        else
            cat = '其他';
        if (!byCategory[cat])
    ...
    if (added.length > 0 && modified.length === 0 && deleted.length === 0)
        suggestedLabels.push('feat');
    if (modified.length > 0 && added.length === 0 && deleted.length === 0)
        suggestedLabels.push('refactor');
    if (added.length > 0 && modified.length > 0)
        suggestedLabels.push('feat');
    if (deleted.length > 0 && added.length === 0)
        suggestedLabels.push('chore');
    if (changes.some(c => c.path.includes('test') || c.path.includes('spec')))
        suggestedLabels.push('test');
    if (changes.some(c => c.path.includes('.md') || c.path.includes('doc') || c.path.includes('README')))
        suggestedLabels.push('docs');
    if (changes.some(c => c.path.includes('fix') || c.path.includes('bug')))
        suggestedLabels.push('fix');
    // Build message parts
    const parts = [];
    ...
    const actionWords = [];
    if (added.length > 0)
        actionWords.push(`新增 ${added.length} 个文件`);
    if (modified.length > 0)
        actionWords.push(`修改 ${modified.length} 个文件`);
    if (deleted.length > 0)
        actionWords.push(`删除 ${deleted.length} 个文件`);
 ...
 */
async function generateAICommitMessage(changes, tool, getDiffContent) {
