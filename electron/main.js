...
`;
    ...
    electron_1.ipcMain.handle('ai-workshop:read-visual-file', async (_, projectPath) => {
        try {
    ...
    electron_1.ipcMain.handle('ai-workshop:scan-project-dirs', async (_, projectPath) => {
        try {
    ...
    electron_1.ipcMain.handle('workshop:ensure-assets', async () => {
    ...
    electron_1.ipcMain.handle('horsefarm:save-data', async (_, projectPath, data) => {
        try {
            const hfPath = path.join(projectPath, '.dbvs-horsefarm.json');
            let existing = {};
            if (await fs.pathExists(hfPath)) {
                try {
                    existing = await fs.readJson(hfPath);
                }
    ...
    electron_1.ipcMain.handle('horsefarm:load-data', async (_, projectPath) => {
        try {
            const hfPath = path.join(projectPath, '.dbvs-horsefarm.json');
            if (!await fs.pathExists(hfPath)) {
    ...
    electron_1.ipcMain.handle('horsefarm:generate-summary', async (_, projectPath, requirements) => {
        try {
    ...
    electron_1.ipcMain.handle('horsefarm:generate-mindmap', async (_, projectPath, summary) => {
        try {
            ...
            if (await fs.pathExists(hfPath)) {
                try {
                    hfData = await fs.readJson(hfPath);
                }
                catch { /* ignore */ }
            }
    ...
    electron_1.ipcMain.handle('horsefarm:generate-kb', async (_, projectPath, projectName, summary, requirements) => {
        try {
            let fileTree = '';
            try {
                const files = await fs.readdir(projectPath);
...
`;
    ...
    electron_1.ipcMain.handle('horsefarm:read-mindmap', async (_, filePath) => {
        try {
            if (!await fs.pathExists(filePath)) {
    ...
    electron_1.ipcMain.handle('horsefarm:read-kb', async (_, projectPath) => {
        try {
            const kbPath = path.join(projectPath, 'DBHT-KNOWLEDGEBASE.md');
    ...
    // Horse Farm — 一键初始化全部项目 (生成 KB + MindMap)
    electron_1.ipcMain.handle('horsefarm:initialize-all', async (_, projects) => {
        const results = [];
        for (const proj of projects) {
            const result = { path: proj.path, name: proj.name, kb: 'skipped', mindmap: 'skipped' };
            try {
                const kbPath = path.join(proj.path, 'DBHT-KNOWLEDGEBASE.md');
                const mindmapPath = path.join(proj.path, '.dbvs-mindmap.json');
                // Check and generate KB
                if (await fs.pathExists(kbPath)) {
                    result.kb = 'exists';
                }
                else {
...
## Development Timeline
- **${new Date().toISOString().split('T')[0]}**: Initialized by Horse Farm
...
`;
                        ...
                        result.kb = 'failed: ' + String(err);
                    }
                }
                // Check and generate MindMap
                if (await fs.pathExists(mindmapPath)) {
                    ...
                    try {
                            ...
                            if (stat.isDirectory()) {
                                try {
                                    ...
                                    for (const sf of subFiles.filter(s => !s.startsWith('.'))) {
                                        childNodes.push({ id: `${f}-${sf}`, label: sf, type: 'file', status: 'pending', progress: 0, children: [] });
                                    }
                                }
                                catch { /* ignore */ }
                            }
                            moduleNodes.push({
                                id: f, label: f, type: stat.isDirectory() ? 'module' : 'file',
                                status: 'pending', progress: 0, children: childNodes,
                        ...
                        const mindMapData = {
                            ...
                            generatedAt: new Date().toISOString(),
                            rootNode: { id: 'root', label: proj.name, type: 'root', status: 'in_progress', progress: 0, children: moduleNodes },
                        };
                        await fs.writeJson(mindmapPath, mindMapData, { spaces: 2 });
                        ...
                        result.mindmap = 'failed: ' + String(err);
                    }
                }
            }
            catch (err) {
                result.kb = 'error';
                result.mindmap = 'error';
            }
            results.push(result);
        }
    ...
    const HF_CONFIG_DIR = path.join(electron_1.app.getPath('userData'), 'horsefarm');
    ...
    electron_1.ipcMain.handle('horsefarm:save-project-ids', async (_, ids) => {
        try {
    ...
    electron_1.ipcMain.handle('horsefarm:load-project-ids', async () => {
        try {
            if (!await fs.pathExists(HF_PROJECT_IDS_PATH))
    ...
    electron_1.ipcMain.handle('horsefarm:save-config', async (_, config) => {
        try {
            await fs.ensureDir(HF_CONFIG_DIR);
    ...
    electron_1.ipcMain.handle('horsefarm:load-config', async () => {
        try {
            ...
            return { success: true, config: { projectIds: [], apiKeys: [], settings: {} } };
        }
    });
